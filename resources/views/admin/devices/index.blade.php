<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ierīču saraksts') }}
        </h2>
    </x-slot>

    

    <div class="py-12">
        <div class="max-w-7x2 mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-end m-2 p-2">
                <a href="{{ route('admin.devices.create') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rouned-lg text-white">Jauna ierīce</a>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left text-gray-500 white:text-gray-400">
    <thead class="text-xs text-gray-700 uppercase bg-gray-50 white:bg-gray-700 white:text-gray-400">
    <tr>
    <th scope="col" class="px-6 py-3">
        @sortablelink('DeviceName','Ierīce')
    </th>
    <th scope="col" class="px-6 py-3">
        @sortablelink('Device','Modelis')
    </th>
    <th scope="col" class="px-6 py-3">
        @sortablelink('manufacturer','Ražotājs')
    </th>
    <th scope="col" class="px-6 py-3">
        @sortablelink('identifier','Identifikators')
    </th>
    <th scope="col" class="px-6 py-3">
        Sērijas numurs
        </th>
        <th scope="col" class="px-6 py-3">
            Pieejams
            </th>
            
    <th scope="col" class="px-6 py-3">
    
    </th>
    </tr>
    </thead>
    <tbody>
    
        @foreach($devices as $device)
        <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700">
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                {{ $device->DeviceName }}
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                {{ $device->Device }}
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                {{ $device->manufacturer}}
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                {{ $device->identifier }}
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                {{ $device->ISN }}
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                Jā
            </td>
            <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap white:text-white">
                <div class="flex space-x-2">
                    <a href="{{ route('admin.devices.edit', $device->id) }}"
                        class="px-4 py-2 bg-green-500 hover:bg-green-700 rounded-lg text-black">Rediģēt</a>
                   <form 
                   
                   class="px-4 py-2 bg-red-500 hover:bg-red-red rounded-lg text-black" 
                   method="POST" 
                   action="{{ route('admin.devices.destroy', $device->id) }}"
                   onsubmit="return confirm('Vai esi parliecināts?');">
                   @csrf
                   @method('DELETE')
                   <button type="submit">Dzēst</button>

                   
                  
                   </form>
                   
                    
                </div>
            </td>
    </tr>
    @endforeach
   
    </tbody>
    </table>
    {!!$devices->appends(\Request::except('page'))->render()!!}
    </div>
    
                </div>
            </div>
        </div>
    </div>
</x-admin-layout>
